...
public class ToolBag {
      ...
      if(!isPageFull[i]){
        ...
        for(byte r=0;r<itemBrdrs[0];r++){ for(byte c=0;c<itemBrdrs[1];c++){
          if(quickStor.itemGrid[r][c]==null){
            ...
            System.out.println("TEST CHECK! i="+i);

            System.out.println("MENU CHECK! "+menu.toString());
            System.out.println("Array Check "+ Arrays.deepToString(quickStor.bagLayout));
            refreshBag();
            quickStor = null;
            return;
  ...
  public static void refreshBag() throws IOException, ClassNotFoundException {
    ...
    System.out.println("Array Check "+ Arrays.deepToString(curPage.bagLayout));
      ...
      for(byte r=0; r<itemGrid.length; r++){
        for(byte c=0; c<itemGrid[0].length; c++){
          if(itemGrid[r][c]==null){
            System.out.println("CHECKER tools "+menu.toString());
